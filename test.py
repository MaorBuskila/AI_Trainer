...
cap = cv2.VideoCapture('ROnny3.MOV')
backSub = cv2.bgsegm.createBackgroundSubtractorMOG()
backSub = cv2.createBackgroundSubtractorMOG2()


# cap = cv2.VideoCapture('http://192.168.1.59:8080/video')
def drawArmsContours(img,fgMask, x12, y12, x14, y14, x16, y16, x13, y13, x15, y15, x17, y17):
    if (x12, y12, x14, y14, x16, y16):
        ...
        arm1_img = img[y1_min:y1_max, x1_min:x1_max]
        ...
        arm2_img = img[y2_min:y2_max, x2_min:x2_max]
        ...
        if arm2_img.size > 0:
            ...
            cv2.drawContours(img[y1_min:y1_max, x1_min:x1_max], contours1, -1, (0, 0, 255), 2)
            cv2.drawContours(img[y2_min:y2_max, x2_min:x2_max], contours2, -1, (0, 0, 255), 2)
...
def main():
    ...
    while True:
        ...
        if len(lmList) != 0:
            ...
            # Left arm
            angle2 = detector.findAngle(img, 13, 11, 23, False)
            # Right elbow
            ...
            drawArmsContours(img,fgMask, x1, y1, x2, y2, x3, y3, x4, y4, x5, y5, x6, y6)
        # # Draw Bar1
